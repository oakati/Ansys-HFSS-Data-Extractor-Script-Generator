...
%%
Header = ['\n'' ----------------------------------------------'...
    '\n'' Script Written by Omer Kati for automation of creating randomly-generated sweeps'...
    '\n'' %s:%s:%s  %s.%s.%s'...
    '\n'' ----------------------------------------------'];
%%
Goals = '), _\nArray("NAME:Goals"))';
...
%%
dataInfo = size(data);
AnsoftApp.Inputs.Size = dataInfo(2)-2;
...
startFormat = strjoin(string(startFormat),'');
AnsoftApp.bytes = AnsoftApp.bytes + fprintf(fileID,startFormat);

%%
% AnsoftApp.bytes = AnsoftApp.bytes + fprintf(fileID,startFormat);
%%
for k=1:ceil((AnsoftApp.Data.Size-1)/10)
    sweepFormat = [
        '\noModule.InsertSetup "OptiParametric", _'...
        '\nArray("NAME:' strcat(AnsoftApp.ParametricSweep,num2str(k)) '", "IsEnabled:=", '...
    ...
    sweepFormat = strjoin(string(sweepFormat),'');
    AnsoftApp.bytes = AnsoftApp.bytes + fprintf(fileID,sweepFormat);
    for j=(10*(k-1)+1):1:(10*k+1)
